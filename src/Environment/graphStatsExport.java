...
public class graphStatsExport 
{
    ...
    public static ChartPanel[] renderGraphs(int _rounds, roundStatsHolder _globalStats, ArrayList<roundStatsHolder> _localStats)
    {
	...
	XYSeries proposersPerRoundSeries = new XYSeries("Proposers");
        ...
        for (int i = 0; i < _rounds; i++)
            agentsPerRoundSeries.add(i, _localStats.get(i)._SAgentsCountPerRound);
        
        for (int i = 0; i < _rounds; i++)
            compositeExpSeries.add(i, _localStats.get(i)._avgExpPerRound, _globalStats.g_stdMinDevianceAVG[i], _globalStats.g_stdPlusDevianceAVG[i]);
        
        for (int i = 0; i < _rounds; i++)
        {
            double _avg = _localStats.get(i)._avgChallengeCountPerRound;
            double _std = _localStats.get(i)._stdDevianceChallenges;
            ...
            compositeProblemSeries.add(i, _localStats.get(i)._avgChallengeCountPerRound,
                                          _localStats.get(i)._avgChallengeCountPerRound - _localStats.get(i)._stdDevianceChallenges, 
                                          _localStats.get(i)._avgChallengeCountPerRound + _localStats.get(i)._stdDevianceChallenges);
        }
        
        for (int i = 0; i < _rounds; i++)
            proposersPerRoundSeries.add(i, _localStats.get(i)._PAgentsCountPerRound);
	...
	domainAxis2.setRange(1, _rounds);
	...
	for (int i = 0; i < _rounds; i++)
            avgAgentsRoundsSeries.add(i + 1, _localStats.get(i).getSolvedAgents());
	...
	domainAxis1.setRange(1, _rounds);
	domainAxis1.setTickUnit(new NumberTickUnit(5));
        
        try {
            if (FactoryHolder._configManager.getStringValue("MODE").equals("console"))
            {
                BufferedImage image = agentsPerRoundChart.createBufferedImage(3000, 2000);
                //ByteArrayOutputStream byteArray = new ByteArrayOutputStream();
                BufferedImage imagecomposite = compositeChart.createBufferedImage(3000, 2000);
                //ChartUtilities.writeBufferedImageAsPNG(byteArray, image);
                
                FactoryHolder._agentsCount.addPicture(image);
                FactoryHolder._compositeChart.addPicture(imagecomposite);
                
            } else {
                ChartUtilities.saveChartAsPNG(new File(FactoryHolder._configManager.getStringValue("GRAPH_OUTPUT_FOLDER") + File.separator
                                                        + OutputNameFormatter.parseName(FactoryHolder._graphNames[2] + "_Merged")), avgAgentsRoundsChart, 3000, 1000);

                ChartUtilities.saveChartAsPNG(new File(FactoryHolder._configManager.getStringValue("GRAPH_OUTPUT_FOLDER") + File.separator
                                                        + OutputNameFormatter.parseName(FactoryHolder._graphNames[1] + "_Merged")), agentsPerRoundChart, 3000, 2000);
                ...
                ChartUtilities.saveChartAsPNG(new File(FactoryHolder._configManager.getStringValue("GRAPH_OUTPUT_FOLDER") + File.separator
                                                        + OutputNameFormatter.parseName(FactoryHolder._graphNames[3] + "_Merged")), compositeChart, 3000, 2000);
            }
